...
void runPigeonIntegrationTests(TargetGenerator targetGenerator) {
  ...
  group('Host sync API tests', () {
    ...
    testWidgets('createAllTypesWithDefaults returns object with correct default values', (
      WidgetTester _,
    ) async {
      final api = HostIntegrationCoreApi();

      final AllTypesWithDefaults createdObject = await api.createAllTypesWithDefaults();
      ...
      expect(immutable.objectMap['doubleMap'], <int, num>{1: 1, 2: 0, 3: 3.0});
      ...
      expect(immutable.listMap[4], <AnEnum>[AnEnum.one, AnEnum.two, AnEnum.three]);
      ...
      expect(immutable.mapMap[3], <AnEnum, String>{AnEnum.one: 'hello', AnEnum.two: 'world'});
      expect(immutable.mapMap[4], <String, AnEnum>{'hello': AnEnum.one, 'world': AnEnum.two});
    ...
    testWidgets('echoAllTypesWithDefaults preserves default values across platforms', (
      WidgetTester _,
    ) async {
      final api = HostIntegrationCoreApi();

      // Test that our test data with defaults roundtrips correctly
      final AllTypesWithDefaults echoedObject = await api.echoAllTypesWithDefaults(
        allTypesWithDefaults,
      );
      expect(echoedObject, allTypesWithDefaults);
    }, skip: !defaultValuesSupportedTargets.contains(targetGenerator));

    testWidgets('default values are correctly generated in target platform', (
      WidgetTester _,
    ) async {
      final api = HostIntegrationCoreApi();

      // Create object with defaults on the platform side
      final AllTypesWithDefaults platformDefaults = await api.createAllTypesWithDefaults();

      // Echo it back to ensure serialization works
      final AllTypesWithDefaults echoedDefaults = await api.echoAllTypesWithDefaults(
        platformDefaults,
      );
      ...
      expect(platformDefaults.allTypes.aBool, allTypesWithDefaults.allTypes.aBool);
      expect(platformDefaults.allTypes.aString, allTypesWithDefaults.allTypes.aString);
      expect(
  ...
  group('Host async API tests', () {
    ...
    testWidgets(
      ...
      (WidgetTester _) async {
        ...
        final AllNullableTypes? echoObject = await api.echoModernAsyncNullableAllNullableTypes(
          recursiveAllNullableTypes,
        );
    ...
    testWidgets('all datatypes async serialize and deserialize correctly using `await`-style', (
      WidgetTester _,
    ) async {
      final api = HostIntegrationCoreApi();

      final AllTypes echoObject = await api.echoModernAsyncAllTypes(genericAllTypes);
    ...
    testWidgets(
      ...
      (WidgetTester _) async {
        ...
        final AllTypes echoObject = await api.echoModernAsyncAllTypesAndNotThrow(genericAllTypes);
    ...
    testWidgets('all datatypes async serialize correctly using `await`-style and throws', (
      WidgetTester _,
    ) async {
      final api = HostIntegrationCoreApi();
...
<<<<<<< HEAD:packages/golubets/platform_tests/shared_test_plugin_code/lib/integration_tests.dart
  ...
  test('nested kotlin sealed classes serialize and deserialize correctly', () async {
    ...
    final states = <SomeState>[Loading(progress: 42), Success(data: 'ok'), Error(code: 7)];
  ...
  const targetSupportsGenerics = <TargetGenerator>[TargetGenerator.kotlin, TargetGenerator.swift];

  // Generic API tests
  group('Host Generic API tests', skip: !targetSupportsGenerics.contains(targetGenerator), () {
    final api = HostGenericApi();

    testWidgets('generic container int echo works', (WidgetTester _) async {
      const input = GenericContainer<int>(value: 42, values: <int>[1, 2, 3]);
    ...
    testWidgets('generic container string echo works', (WidgetTester _) async {
      const input = GenericContainer<String>(value: 'test', values: <String>['a', 'b', 'c']);
      final GenericContainer<String> result = await api.echoGenericString(input);
      expect(result.value, equals(input.value));
      expect(result.values, equals(input.values));
    });

    testWidgets('generic container double echo works', (WidgetTester _) async {
      const input = GenericContainer<double>(value: 3.14, values: <double>[1.0, 2.0, 3.0]);
      final GenericContainer<double> result = await api.echoGenericDouble(input);
      expect(result.value, equals(input.value));
      expect(result.values, equals(input.values));
    });

    testWidgets('generic container bool echo works', (WidgetTester _) async {
      const input = GenericContainer<bool>(value: true, values: <bool>[true, false, true]);
    ...
    testWidgets('generic container enum echo works', (WidgetTester _) async {
      const input = GenericContainer<GenericsAnEnum>(
        ...
        values: <GenericsAnEnum>[GenericsAnEnum.one, GenericsAnEnum.two, GenericsAnEnum.three],
      );
      final GenericContainer<GenericsAnEnum> result = await api.echoGenericEnum(input);
      expect(result.value, equals(input.value));
      expect(result.values, equals(input.values));
    });

    testWidgets('generic container nullable int echo works', (WidgetTester _) async {
      const input = GenericContainer<int?>(value: 42, values: <int?>[1, null, 3]);
      final GenericContainer<int?> result = await api.echoGenericNullableInt(input);
      expect(result.value, equals(input.value));
      expect(result.values, equals(input.values));
    });

    testWidgets('generic container nullable string echo works', (WidgetTester _) async {
      const input = GenericContainer<String?>(value: 'test', values: <String?>['a', null, 'c']);
      final GenericContainer<String?> result = await api.echoGenericNullableString(input);
      expect(result.value, equals(input.value));
      expect(result.values, equals(input.values));
    });
    ...
    testWidgets('generic pair string-int echo works', (WidgetTester _) async {
      ...
      final GenericPair<String, int> result = await api.echoGenericPairStringInt(input);
    ...
    testWidgets('generic pair int-string echo works', (WidgetTester _) async {
      ...
      final GenericPair<int, String> result = await api.echoGenericPairIntString(input);
      expect(result.first, equals(input.first));
      expect(result.second, equals(input.second));
      expect(result.map, equals(input.map));
    });

    testWidgets('generic pair double-bool echo works', (WidgetTester _) async {
      final input = GenericPair<double, bool>(
        first: 3.14,
        second: true,
        map: <double, bool>{1.0: true, 2.0: false},
      );
      final GenericPair<double, bool> result = await api.echoGenericPairDoubleBool(input);
      expect(result.first, equals(input.first));
      expect(result.second, equals(input.second));
      expect(result.map, equals(input.map));
    });

    testWidgets('nested generic string-int-double echo works', (WidgetTester _) async {
      const input = NestedGeneric<String, int, double>(
        container: GenericContainer<String>(value: 'test', values: <String>['a', 'b']),
        pairs: <GenericPair<int, double>>[
          GenericPair<int, double>(first: 1, second: 1.0, map: <int, double>{1: 1.0, 2: 2.0}),
        ],
        nestedMap: <String, GenericContainer<int>>{
          'key': GenericContainer<int>(value: 42, values: <int>[1, 2, 3]),
        },
        listOfMaps: <Map<int, double>>[
          <int, double>{1: 1.0, 2: 2.0},
        ],
      );
      final NestedGeneric<String, int, double> result = await api.echoNestedGenericStringIntDouble(
        input,
      );
    ...
    testWidgets('generic typed list in Either works', (WidgetTester _) async {
      const input = Right<GenericsAnEnum, List<SimpleStringContainer>>(
        value: [
          SimpleStringContainer(id: '1', name: 'first'),
          SimpleStringContainer(id: '2', name: 'second'),
        ],
      );

      final Either<GenericsAnEnum, List<SimpleStringContainer>> result = await api
          .echoFlutterReturnEitherGenericEnumOrListStringContainer(input);
      expect(result, isA<Right<GenericsAnEnum, List<SimpleStringContainer>>>());
    });
    ...
    testWidgets('generic list container echo works', (WidgetTester _) async {
      ...
      final List<GenericContainer<int>> result = await api.echoListGenericContainer(input);
    ...
    testWidgets('generic list pair echo works', (WidgetTester _) async {
      final input = <GenericPair<String, int>>[
        const GenericPair<String, int>(first: 'a', second: 1, map: <String, int>{'x': 1}),
        const GenericPair<String, int>(first: 'b', second: 2, map: <String, int>{'y': 2}),
      ];
      final List<GenericPair<String, int>> result = await api.echoListGenericPair(input);
      expect(result.length, equals(input.length));
      for (var i = 0; i < result.length; i++) {
    ...
    testWidgets('generic map container echo works', (WidgetTester _) async {
      ...
      final Map<String, GenericContainer<int>> result = await api.echoMapGenericContainer(input);
      expect(result.length, equals(input.length));
    ...
    testWidgets('generic map pair echo works', (WidgetTester _) async {
      ...
      final Map<int, GenericPair<String, double>> result = await api.echoMapGenericPair(input);
      expect(result.length, equals(input.length));
    ...
    testWidgets('async generic int echo works', (WidgetTester _) async {
      const input = GenericContainer<int>(value: 42, values: <int>[1, 2, 3]);
      final GenericContainer<int> result = await api.echoAsyncGenericInt(input);
      expect(result.value, equals(input.value));
      expect(result.values, equals(input.values));
    });
    ...
    testWidgets('async nested generic echo works', (WidgetTester _) async {
      const input = NestedGeneric<String, int, double>(
        container: GenericContainer<String>(value: 'test', values: <String>['a']),
        pairs: <GenericPair<int, double>>[
          GenericPair<int, double>(first: 1, second: 1.0, map: <int, double>{1: 1.0}),
        ],
        nestedMap: <String, GenericContainer<int>>{
      ...
      final NestedGeneric<String, int, double> result = await api.echoAsyncNestedGeneric(input);
      expect(result.container.value, equals(input.container.value));
    });

    testWidgets('either generic container echo works', (WidgetTester _) async {
      ...
      final Either<GenericContainer<int>, GenericContainer<String>> result = await api
          .echoEitherGenericIntOrString(input);
      expect(result, isA<Left<GenericContainer<int>, GenericContainer<String>>>());
      final leftResult = result as Left<GenericContainer<int>, GenericContainer<String>>;
    ...
    testWidgets('either generic pair echo works', (WidgetTester _) async {
      ...
      final Either<GenericPair<String, int>, GenericPair<int, String>> result = await api
          .echoEitherGenericPairStringIntOrIntString(input);
      expect(result, isA<Right<GenericPair<String, int>, GenericPair<int, String>>>());
      final rightResult = result as Right<GenericPair<String, int>, GenericPair<int, String>>;
      expect(rightResult.value.first, equals(42));
      expect(rightResult.value.second, equals('test'));
    });

    // GenericsAllNullableTypesTyped tests
    testWidgets('typed nullable string-int-double echo works', (WidgetTester _) async {
      final input = GenericsAllNullableTypesTyped<String, int, double>(
        ...
        aNullableFloatArray: Float64List.fromList(<double>[1.1, 2.2, 3.3]),
        ...
        recursiveClassList: <GenericsAllNullableTypes?>[null],
        map: <String?, String?>{'key': 'value'},
        stringMap: <String?, String?>{'s1': 's2'},
        intMap: <double?, int?>{1.0: 10},
        enumMap: <GenericsAnEnum?, GenericsAnEnum?>{GenericsAnEnum.one: GenericsAnEnum.two},
        ...
        recursiveClassMap: <int?, GenericsAllNullableTypes?>{1: null},
      );
      final GenericsAllNullableTypesTyped<String, int, double> result = await api
          .echoTypedNullableStringIntDouble(input);
    ...
    testWidgets('typed nullable int-string-bool echo works', (WidgetTester _) async {
      final input = GenericsAllNullableTypesTyped<int, String, bool>();
      final GenericsAllNullableTypesTyped<int, String, bool> result = await api
          .echoTypedNullableIntStringBool(input);
    ...
    testWidgets('typed nullable enum-double-string echo works', (WidgetTester _) async {
      final input = GenericsAllNullableTypesTyped<GenericsAnEnum, double, String>(
        aNullableBool: false,
        aNullableInt: 100,
        aNullableInt64: 200,
        aNullableDouble: 2.71,
        aNullableByteArray: Uint8List.fromList(<int>[9, 8, 7]),
        aNullable4ByteArray: Int32List.fromList(<int>[6, 5, 4]),
        aNullable8ByteArray: Int64List.fromList(<int>[3, 2, 1]),
        aNullableFloatArray: Float64List.fromList(<double>[4.4, 5.5]),
        ...
        recursiveClassList: <GenericsAllNullableTypes?>[null],
        map: <GenericsAnEnum?, GenericsAnEnum?>{GenericsAnEnum.one: GenericsAnEnum.two},
        ...
        recursiveClassMap: <int?, GenericsAllNullableTypes?>{2: null},
      );
      final GenericsAllNullableTypesTyped<GenericsAnEnum, double, String> result = await api
          .echoTypedNullableEnumDoubleString(input);
      expect(result.aNullableBool, equals(input.aNullableBool));
      expect(result.aNullableString, equals(input.aNullableString));
      expect(result.aNullableEnum, equals(input.aNullableEnum));
    });

    testWidgets('generic container with typed nullable echo works', (WidgetTester _) async {
      ...
      final input = GenericContainer<GenericsAllNullableTypesTyped<String, int, double>>(
        value: typedInput,
        values: <GenericsAllNullableTypesTyped<String, int, double>>[typedInput],
      );
      final GenericContainer<GenericsAllNullableTypesTyped<String, int, double>> result = await api
          .echoGenericContainerTypedNullable(input);
      expect(result.value?.aNullableString, equals(typedInput.aNullableString));
    ...
    testWidgets('list of typed nullable echo works', (WidgetTester _) async {
      ...
      final input = <GenericsAllNullableTypesTyped<String, int, double>>[typed1, typed2];
      final List<GenericsAllNullableTypesTyped<String, int, double>> result = await api
          .echoListTypedNullable(input);
    ...
    testWidgets('map with typed nullable echo works', (WidgetTester _) async {
      ...
      final input = <String, GenericsAllNullableTypesTyped<int, String, double>>{
        'key1': typedValue,
      };
      final Map<String, GenericsAllNullableTypesTyped<int, String, double>> result = await api
          .echoMapTypedNullable(input);
      expect(result.containsKey('key1'), isTrue);
      expect(result['key1']?.aNullableString, equals('map-test'));
    });

    testWidgets('async typed nullable string-int-double echo works', (WidgetTester _) async {
      final input = GenericsAllNullableTypesTyped<String, int, double>(
        aNullableBool: true,
        aNullableInt: 42,
        aNullableInt64: 64,
      ...
      final GenericsAllNullableTypesTyped<String, int, double> result = await api
          .echoAsyncTypedNullableStringIntDouble(input);
      expect(result.aNullableBool, equals(input.aNullableBool));
      expect(result.aNullableString, equals(input.aNullableString));
    });

    testWidgets('async generic container typed nullable echo works', (WidgetTester _) async {
      final typedInput = GenericsAllNullableTypesTyped<String, int, double>(
        aNullableBool: false,
        aNullableInt: 123,
        aNullableInt64: 456,
      ...
      final GenericContainer<GenericsAllNullableTypesTyped<String, int, double>> result = await api
          .echoAsyncGenericContainerTypedNullable(input);
    ...
    testWidgets('GenericDefaults echo works', (WidgetTester _) async {
      ...
      final input = GenericDefaults(
        genericInt: const GenericContainer<int>(value: 100, values: <int>[10, 20, 30]),
        genericString: const GenericContainer<String>(
          value: 'test',
          values: <String>['x', 'y', 'z'],
        ),
        genericDouble: const GenericContainer<double>(value: 2.5, values: <double>[1.1, 2.2, 3.3]),
        genericBool: const GenericContainer<bool>(value: false, values: <bool>[false, true, false]),
        ...
        nestedGenericDefault: const NestedGeneric<String, int, double>(
          ...
          pairs: <GenericPair<int, double>>[
            GenericPair<int, double>(first: 5, second: 5.5, map: <int, double>{5: 5.5, 6: 6.6}),
          ],
          nestedMap: <String, GenericContainer<int>>{
            'test-nested': GenericContainer<int>(value: 55, values: <int>[50, 60, 70]),
          },
          listOfMaps: <Map<int, double>>[
      ...
      expect(result.genericPairStringInt.first, equals(input.genericPairStringInt.first));
      expect(result.genericPairStringInt.second, equals(input.genericPairStringInt.second));
      expect(result.genericPairStringInt.map, equals(input.genericPairStringInt.map));

      expect(result.genericPairIntString.first, equals(input.genericPairIntString.first));
      expect(result.genericPairIntString.second, equals(input.genericPairIntString.second));
      expect(result.genericPairIntString.map, equals(input.genericPairIntString.map));
    ...
    testWidgets('GenericDefaults returnGenericDefaults works', (WidgetTester _) async {
      final api = HostGenericApi();
      ...
      expect(result.nestedGenericDefault.container.values, equals(<String>['x', 'y', 'z']));
    });
    ...
    testWidgets('GenericDefaults async echo works', (WidgetTester _) async {
      ...
      final input = GenericDefaults();
      final GenericDefaults result = await api.echoAsyncGenericDefaults(input);
  ...
  group('FlutterGenericApi tests', skip: !targetSupportsGenerics.contains(targetGenerator), () {
    setUp(() {
      FlutterGenericApi.setUp(_FlutterGenericApiTestImplementation());
    });

    testWidgets('generic containers serialize and deserialize correctly', (WidgetTester _) async {
      final api = HostGenericApi();

      const sentContainer = GenericContainer<int>(value: 42, values: <int>[1, 2, 3]);
      final GenericContainer<int> echoContainer = await api.callFlutterEchoGenericInt(
        sentContainer,
      );
    ...
    testWidgets('generic string containers serialize and deserialize correctly', (
      WidgetTester _,
    ) async {
      final api = HostGenericApi();
      ...
      final GenericContainer<String> echoContainer = await api.callFlutterEchoGenericString(
        sentContainer,
      );
      expect(echoContainer.value, sentContainer.value);
      expect(echoContainer.values, sentContainer.values);
    ...
    testWidgets('generic pairs serialize and deserialize correctly', (WidgetTester _) async {
      final api = HostGenericApi();
      ...
      final GenericPair<String, int> echoPair = await api.callFlutterEchoGenericPairStringInt(
        sentPair,
      );
    ...
    testWidgets('generic defaults serialize and deserialize correctly', (WidgetTester _) async {
      final api = HostGenericApi();

      final sentDefaults = GenericDefaults();
      final GenericDefaults echoDefaults = await api.callFlutterEchoGenericDefaults(sentDefaults);
      ...
      expect(echoDefaults.genericString.value, sentDefaults.genericString.value);
      expect(echoDefaults.genericString.values, sentDefaults.genericString.values);
    });

    testWidgets('generic defaults int extraction works correctly', (WidgetTester _) async {
      final api = HostGenericApi();

      final sentDefaults = GenericDefaults();
      final GenericContainer<int> echoContainer = await api.callFlutterEchoGenericDefaultsInt(
        sentDefaults,
      );
    ...
    testWidgets('nested generics serialize and deserialize correctly', (WidgetTester _) async {
      final api = HostGenericApi();

      final sentDefaults = GenericDefaults();
      final NestedGeneric<String, int, double> echoNested = await api
          .callFlutterEchoGenericDefaultsNested(sentDefaults);
      expect(echoNested.container.value, sentDefaults.nestedGenericDefault.container.value);
      expect(echoNested.container.values, sentDefaults.nestedGenericDefault.container.values);
      expect(echoNested.pairs.length, sentDefaults.nestedGenericDefault.pairs.length);
    });

    testWidgets('generic pair either extraction works correctly', (WidgetTester _) async {
      final api = HostGenericApi();
    ...
    testWidgets('typed nullables serialize and deserialize correctly', (WidgetTester _) async {
      final api = HostGenericApi();
      ...
      final GenericsAllNullableTypesTyped<String, int, double> echoTyped = await api
          .callFlutterEchoTypedNullableStringIntDouble(sentTyped);
    ...
    testWidgets('typed nullables with different type params work correctly', (
      ...
      final GenericsAllNullableTypesTyped<int, String, bool> echoTyped = await api
          .callFlutterEchoTypedNullableIntStringBool(sentTyped);
      expect(echoTyped.aNullableString, sentTyped.aNullableString);
      expect(echoTyped.aNullableInt, sentTyped.aNullableInt);
      expect(echoTyped.aNullableDouble, sentTyped.aNullableDouble);
    });

    testWidgets('nested generics with specific types work correctly', (WidgetTester _) async {
      final api = HostGenericApi();
    ...
    testWidgets('generic container typed nullable works correctly', (WidgetTester _) async {
      final api = HostGenericApi();

      final sentContainer = GenericContainer<GenericsAllNullableTypesTyped<String, int, double>>(
      ...
      final GenericContainer<GenericsAllNullableTypesTyped<String, int, double>> echoContainer =
          await api.callFlutterEchoGenericContainerTypedNullable(sentContainer);
      expect(echoContainer.value?.aNullableString, sentContainer.value?.aNullableString);
      expect(echoContainer.value?.aNullableInt, sentContainer.value?.aNullableInt);
      expect(echoContainer.values.length, sentContainer.values.length);
    });

    testWidgets('list of generic containers works correctly', (WidgetTester _) async {
      final api = HostGenericApi();
      ...
      final List<GenericContainer<int>> echoList = await api.callFlutterEchoListGenericContainer(
        sentList,
      );
    ...
    testWidgets('list of typed nullables works correctly', (WidgetTester _) async {
      final api = HostGenericApi();
      ...
      final List<GenericsAllNullableTypesTyped<String, int, double>> echoList = await api
          .callFlutterEchoListTypedNullable(sentList);
      expect(echoList.length, sentList.length);
      expect(echoList[0].aNullableString, sentList[0].aNullableString);
      expect(echoList[1].aNullableInt, sentList[1].aNullableInt);
    });

    testWidgets('map of generic containers works correctly', (WidgetTester _) async {
      final api = HostGenericApi();
    ...
    testWidgets('map of typed nullables works correctly', (WidgetTester _) async {
      final api = HostGenericApi();

      final sentMap = <String, GenericsAllNullableTypesTyped<int, String, double>>{
      ...
      final Map<String, GenericsAllNullableTypesTyped<int, String, double>> echoMap = await api
          .callFlutterEchoMapTypedNullable(sentMap);
      expect(echoMap.length, sentMap.length);
      expect(echoMap['key1']?.aNullableString, sentMap['key1']?.aNullableString);
      expect(echoMap['key2']?.aNullableInt, sentMap['key2']?.aNullableInt);
    });

    testWidgets('return generic defaults either left works correctly', (WidgetTester _) async {
      final api = HostGenericApi();

      final GenericContainer<Either<String, int>> result = await api
          .callFlutterReturnGenericDefaultsEitherLeft();
      expect(result.value, predicate((Left<String, int> v) => v.value == 'default-left'));
      expect(result.values.length, 2);
    });

    testWidgets('return generic defaults either right works correctly', (WidgetTester _) async {
      final api = HostGenericApi();
