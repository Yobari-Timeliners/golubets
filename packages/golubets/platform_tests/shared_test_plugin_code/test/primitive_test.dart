...
void main() {
  test('test anInt', () async {
    ...
    echoOneArgument(
      ...
      PrimitiveHostApi.golubetsChannelCodec,
    );
  ...
  test('test List<bool>', () async {
    ...
    echoOneArgument(
      mockMessenger,
      'dev.bayori.golubets.golubets_integration_tests.PrimitiveHostApi.aBoolList',
      PrimitiveHostApi.golubetsChannelCodec,
    );
    final api = PrimitiveHostApi(binaryMessenger: mockMessenger);
  ...
  test('test List<bool> flutterapi', () async {
    ...
    when(
      mockMessenger.setMessageHandler(
        'dev.bayori.golubets.golubets_integration_tests.PrimitiveFlutterApi.aBoolList',
        any,
      ),
    ).thenAnswer((Invocation realInvocation) {
      final handler = realInvocation.positionalArguments[1] as MessageHandler?;
      handler!(
        PrimitiveFlutterApi.golubetsChannelCodec.encodeMessage(<Object?>[
          <Object?>[true, false],
        ]),
      );
    });
