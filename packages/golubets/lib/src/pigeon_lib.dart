...
class ConfigureGolubets {
  ...
  /// The [GolubetsOptions] that will be merged into the command line options.
...
class GolubetsOptions {
  /// Creates a instance of GolubetsOptions
  const GolubetsOptions({
    this.input,
    this.dartOut,
  ...
  /// [GolubetsOptions].
  GolubetsOptions merge(GolubetsOptions options) {
    return GolubetsOptions.fromMap(mergeMaps(toMap(), options.toMap()));
  }
...
class Golubets {
  /// Create and setup a [Golubets] instance.
  static Golubets setup() {
    return Golubets();
  }
  ...
  static String get usage {
    return '''
Golubets is a tool for generating type-safe communication code between Flutter
and the host platform.

usage: golubets --input <golubets path> --dart_out <dart path> [option]*
      ...
      for (final DeclarationMirror declaration in library.declarations.values) {
        if (declaration is MethodMirror &&
          ...
          if (declaration.parameters.length == 1 &&
            ...
            print("warning: invalid 'configureGolubets' method defined.");
          }
        }
      }
    }
...
class ParseResults {
  ...
  /// The Map representation of any [GolubetsOptions] specified with
  /// [ConfigureGolubets] during parsing.
  final Map<String, Object>? golubetsOptions;
}
