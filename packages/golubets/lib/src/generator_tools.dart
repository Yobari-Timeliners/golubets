...
String makeChannelNameWithStrings({
  ...
  return 'dev.bayori.golubets.$dartPackageName.$apiName.$methodName';
}
...
/// used internally, to avoid churn in Golubets test files.
...
const String generatedCodeWarning =
    'Autogenerated from Golubets (v$golubetsVersion), do not edit directly.';

/// Warning printed at the top of all generated code.
String getGeneratedCodeWarning() {
  final versionString = includeVersionInGeneratedWarning ? ' (v$golubetsVersion)' : '';
  return 'Autogenerated from Golubets$versionString, do not edit directly.';
}

/// String to be printed after `getGeneratedCodeWarning()'s warning`.
const String seeAlsoWarning = 'See also: https://pub.dev/packages/golubets';

/// Prefix for generated internal classes.
///
/// This lowers the chances of variable name collisions with user defined
/// parameters.
const String classNamePrefix = 'GolubetsInternal';

/// Prefix for utility classes generated to be used with ProxyAPIs.
///
/// This lowers the chances of variable name collisions with user defined
/// parameters.
const String proxyApiClassNamePrefix = 'Golubets';
...
/// This lowers the chances of variable name collisions with user defined
/// parameters.
const String classMemberNamePrefix = 'golubets_';

/// Prefix for variable names not defined by the user.
///
/// This lowers the chances of variable name collisions with user defined
/// parameters.
const String varNamePrefix = 'golubetsVar_';
...
TypeDeclaration substituteTypeParameters(
  ...
  final List<TypeDeclaration> substitutedArgs = type.typeArguments
      .map((TypeDeclaration arg) => substituteTypeParameters(arg, typeParameterMap))
      .toList();
...
void _collectNestedGenericTypes(
  ...
  final Class? target = classes.firstWhereOrNull((Class cls) => cls.name == concreteType.baseName);
  ...
  final typeParameterMap = <String, TypeDeclaration>{};
  for (var i = 0; i < target.typeArguments.length && i < concreteType.typeArguments.length; i++) {
    typeParameterMap[target.typeArguments[i].baseName] = concreteType.typeArguments[i];
  }

  // Process each field in the class and substitute type parameters
  for (final NamedType field in target.fields) {
    final TypeDeclaration substitutedType = substituteTypeParameters(field.type, typeParameterMap);
...
Map<String, Set<TypeArgumentCombination>> collectGenericTypeUsage({
  ...
  for (final cls in classes) {
    if (cls.superClassName != null && cls.superClassName!.isNotEmpty) {
      ...
      if (potentialSuperClass != null && potentialSuperClass.isSealed) {
        sealedSubclasses.putIfAbsent(cls.superClassName!, () => <String>[]).add(cls.name);
      }
    }
  }
  ...
  void collectFromType(TypeDeclaration type) {
    ...
    if (subclassNames != null) {
      for (final String subclassName in subclassNames) {
        ...
        if (subclass != null && subclass.typeArguments.isNotEmpty) {
          final subclassType = TypeDeclaration(
            baseName: subclassName,
            isNullable: false,
            typeArguments: combination,
          );
          _collectNestedGenericTypes(classes, subclassType, classToTypeArgs, collectFromType);
        }
      }
    }
    ...
    _collectNestedGenericTypes(classes, type, classToTypeArgs, collectFromType);
  }
...
Iterable<EnumeratedType> getEnumeratedTypes(Root root, {bool excludeSealedClasses = false}) sync* {
  ...
  final Map<String, Set<TypeArgumentCombination>> genericUsage = root.genericUsage;

  for (final Class customClass in root.classes) {
    if (excludeSealedClasses && customClass.isSealed && customClass.superClass == null) {
    ...
    final Set<TypeArgumentCombination>? typeArgCombinations = genericUsage[className];
      ...
      if (typeArgCombinations != null && typeArgCombinations.isNotEmpty) {
        ...
        for (final TypeArgumentCombination combination in typeArgCombinations) {
          yield EnumeratedType(
            ...
            typeArguments: combination,
            substitutedFieldTypes: _substituteFieldTypesForCombination(customClass, combination),
          );
          index += 1;
        }
      }
...
Iterable<NamedType> getFieldsInSerializationOrder(Class classDefinition) {
  // This returns the fields in the order they are declared in the golubets file.
...
extension StringExtension on String {
  ...
  String purify(String input) {
    ...
    final Set<String> targetWords = pattern.allMatches(this).map((m) => m.group(0)!).toSet();
    final Set<String> inputWords = pattern.allMatches(input).map((m) => m.group(0)!).toSet();
...
extension EnumeratedTypeExtensions on EnumeratedType {
  ...
  ({Class child, Class superClass})? findSealedHierarchy() {
    ...
    final bool isSealedChild = associatedClass != null && superClass != null && superClass.isSealed;
