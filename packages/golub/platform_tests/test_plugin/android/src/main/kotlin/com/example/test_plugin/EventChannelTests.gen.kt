...
interface SealedClassApi {
  ...
  companion object {
    ...
    val codec: MessageCodec<Any?> by lazy {
      EventChannelTestsPigeonCodec()
    }
    /** Sets up an instance of `SealedClassApi` to handle messages through the `binaryMessenger`. */
    @JvmOverloads
    fun setUp(binaryMessenger: BinaryMessenger, api: SealedClassApi?, messageChannelSuffix: String = "") {
      val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.pigeon_integration_tests.SealedClassApi.echo$separatedMessageChannelSuffix", codec)
        if (api != null) {
