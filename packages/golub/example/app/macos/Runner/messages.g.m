...
+ (instancetype)makeWithName:(nullable NSString *)name
  ...
  PGNMessageData *golubResult = [[PGNMessageData alloc] init];
  golubResult.name = name;
  golubResult.description = description;
  golubResult.code = code;
  golubResult.data = data;
  return golubResult;
}
+ (PGNMessageData *)fromList:(NSArray<id> *)list {
  PGNMessageData *golubResult = [[PGNMessageData alloc] init];
  golubResult.name = GetNullableObjectAtIndex(list, 0);
  golubResult.description = GetNullableObjectAtIndex(list, 1);
  PGNCodeBox *boxedPGNCode = GetNullableObjectAtIndex(list, 2);
  golubResult.code = boxedPGNCode.value;
  golubResult.data = GetNullableObjectAtIndex(list, 3);
  return golubResult;
}
+ (nullable PGNMessageData *)nullableFromList:(NSArray<id> *)list {
...
@interface PGNMessagesGolubCodecReader : FlutterStandardReader
@end
@implementation PGNMessagesGolubCodecReader
...
@interface PGNMessagesGolubCodecWriter : FlutterStandardWriter
@end
@implementation PGNMessagesGolubCodecWriter
...
@interface PGNMessagesGolubCodecReaderWriter : FlutterStandardReaderWriter
@end
@implementation PGNMessagesGolubCodecReaderWriter
- (FlutterStandardWriter *)writerWithData:(NSMutableData *)data {
  return [[PGNMessagesGolubCodecWriter alloc] initWithData:data];
}
- (FlutterStandardReader *)readerWithData:(NSData *)data {
  return [[PGNMessagesGolubCodecReader alloc] initWithData:data];
}
@end
...
NSObject<FlutterMessageCodec> *PGNGetMessagesCodec(void) {
  ...
  dispatch_once(&sPred, ^{
    PGNMessagesGolubCodecReaderWriter *readerWriter =
        [[PGNMessagesGolubCodecReaderWriter alloc] init];
...
void SetUpPGNExampleHostApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger,
...
#endif
  {
    FlutterBasicMessageChannel *channel = [[FlutterBasicMessageChannel alloc]
           initWithName:[NSString stringWithFormat:@"%@%@",
                                                   @"dev.bayori.golub.golub_example_package."
  ...
  }
  {
    FlutterBasicMessageChannel *channel = [[FlutterBasicMessageChannel alloc]
           initWithName:[NSString stringWithFormat:
                                      @"%@%@",
                                      @"dev.bayori.golub.golub_example_package.ExampleHostApi.add",
                                      messageChannelSuffix]
        binaryMessenger:binaryMessenger
                  codec:PGNGetMessagesCodec()];
    if (api) {
