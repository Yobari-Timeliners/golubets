...
private object MessagesGolubUtils {
...
data class MessageData(
  ...
  companion object {
    fun fromList(golubVar_list: List<Any?>): MessageData {
      val name = golubVar_list[0] as String?
      val description = golubVar_list[1] as String?
      val code = golubVar_list[2] as Code
      val data = golubVar_list[3] as Map<String, String>
  ...
  override fun equals(other: Any?): Boolean {
    ...
    return MessagesGolubUtils.deepEquals(toList(), other.toList())
  }

  override fun hashCode(): Int = toList().hashCode()
}

private open class MessagesGolubCodec : StandardMessageCodec() {
...
interface ExampleHostApi {
  ...
  companion object {
    ...
    val codec: MessageCodec<Any?> by lazy { MessagesGolubCodec() }
    ...
    fun setUp(
      ...
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.bayori.golub.golub_example_package.ExampleHostApi.getHostLanguage$separatedMessageChannelSuffix",
                codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> =
                try {
                  ...
                  MessagesGolubUtils.wrapError(exception)
                }
            reply.reply(wrapped)
          }
        } else {
      ...
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.bayori.golub.golub_example_package.ExampleHostApi.add$separatedMessageChannelSuffix",
                codec)
        if (api != null) {
      ...
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.bayori.golub.golub_example_package.ExampleHostApi.sendMessage$separatedMessageChannelSuffix",
                codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            ...
            api.sendMessage(messageArg) { result: Result<Boolean> ->
              ...
              if (error != null) {
                reply.reply(MessagesGolubUtils.wrapError(error))
              } else {
                val data = result.getOrNull()
                reply.reply(MessagesGolubUtils.wrapResult(data))
              }
            }
          }
        } else {
      ...
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.bayori.golub.golub_example_package.ExampleHostApi.sendMessageModernAsync$separatedMessageChannelSuffix",
                codec,
                taskQueue)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
      ...
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.bayori.golub.golub_example_package.ExampleHostApi.sendMessageModernAsyncThrows$separatedMessageChannelSuffix",
                codec,
                taskQueue)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
...
class MessageFlutterApi(
  ...
  fun flutterMethod(aStringArg: String?, callback: (Result<String>) -> Unit) {
    val separatedMessageChannelSuffix =
        if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName =
        "dev.bayori.golub.golub_example_package.MessageFlutterApi.flutterMethod$separatedMessageChannelSuffix"
    ...
    channel.send(listOf(aStringArg)) {
      if (it is List<*>) {
        ...
        callback(Result.failure(MessagesGolubUtils.createConnectionError(channelName)))
      }
    }
  }
}
