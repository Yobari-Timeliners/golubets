...
private object EventChannelMessagesGolubUtils {
...
data class IntEvent(val data: Long) : PlatformEvent() {
  companion object {
    fun fromList(golubVar_list: List<Any?>): IntEvent {
      val data = golubVar_list[0] as Long
  ...
  override fun equals(other: Any?): Boolean {
    ...
    return EventChannelMessagesGolubUtils.deepEquals(toList(), other.toList())
  }
...
data class StringEvent(val data: String) : PlatformEvent() {
  companion object {
    fun fromList(golubVar_list: List<Any?>): StringEvent {
      val data = golubVar_list[0] as String
...
private open class EventChannelMessagesGolubCodec : StandardMessageCodec() {
...
val EventChannelMessagesGolubMethodCodec = StandardMethodCodec(EventChannelMessagesGolubCodec())

private class EventChannelMessagesGolubStreamHandler<T>(
    val wrapper: EventChannelMessagesGolubEventChannelWrapper<T>
...
interface EventChannelMessagesGolubEventChannelWrapper<T> {
...
abstract class StreamEventsStreamHandler :
    EventChannelMessagesGolubEventChannelWrapper<PlatformEvent> {
  companion object {
    fun register(
      ...
      var channelName: String =
          "dev.bayori.golub.golub_example_package.EventChannelMethods.streamEvents"
      if (instanceName.isNotEmpty()) {
        channelName += ".$instanceName"
      }
      val internalStreamHandler =
          EventChannelMessagesGolubStreamHandler<PlatformEvent>(streamHandler)
      EventChannel(messenger, channelName, EventChannelMessagesGolubMethodCodec)
